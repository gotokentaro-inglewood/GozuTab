...
func main() {
	...
	if dbStatus == "success" {
		...
		CREATE TABLE IF NOT EXISTS tabs (
			...
			satus TEXT,                              -- タブ譜の状態（例: "public", "private"）
			created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 作成日時
		...
		insertTab := `INSERT INTO tabs (user_id, title, artist, content, audio_url, satus) VALUES ($1, $2, $3, $4, $5, $6);`
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Printf("Error fetching user name: %v\n", err)
		}

		fmt.Fprintf(w, "<h1>GozuTab Server</h1>")
		fmt.Fprintf(w, "<p>Hello, %s! Database connection status: %s</p>", name, dbStatus)
	})
